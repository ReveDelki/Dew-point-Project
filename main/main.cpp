...
static String net_Mode = "AP";   // "AP" or "STA"
static String net_IP   = "192.168.4.1";
static bool apStarted = false;
static bool staRetryStarted = false;
...
// Fan control logic: turn ON if (dewA - dewB) > DELTA_ON_HYST, turn OFF if (dewA - dewB) <= DELTA_OFF_EDGE
...
// showIpScreen: helper to show IP and mode on TFT (called at startup)
static void showIpScreen(const String& modeStr, const String& ipStr){
  screen.fillSprite(TFT_BLACK);
  drawCentered("WiFi " + modeStr, SCR_W/2, 12, 2, TFT_WHITE);
  drawCentered("IP: " + ipStr,    SCR_W/2, 30, 2, (modeStr=="STA") ? TFT_GREEN : TFT_ORANGE);
...
// tryNtpSync: try to sync time via NTP, returns true if successful within timeout
...
// ------- Wifi helpers -------
// onWiFiEvent: handle WiFi events (IP acquired, disconnected etc.) to update screen and switch between STA/AP modes
static void onWiFiEvent(WiFiEvent_t event) {
  if (event == ARDUINO_EVENT_WIFI_STA_GOT_IP) {
    net_Mode = "STA";
    net_IP   = WiFi.localIP().toString();
    staRetryStarted = false;
    showIpScreen(net_Mode, net_IP);
    delay(3000);
    ...
    // If AP has not been started yet in this boot, start it now
    if (!apStarted) {
      WiFi.mode(WIFI_AP_STA);
      WiFi.softAP("STH4x_Monitor", "12345678");
      apStarted = true;
    }

    // Show AP screen only when switching from STA -> AP
    if (net_Mode != "AP") {
      net_Mode = "AP";
      net_IP   = WiFi.softAPIP().toString();
      showIpScreen(net_Mode, net_IP);
      delay(3000);
    } else {
      net_IP = WiFi.softAPIP().toString();
    ...
    // Start STA retry only once until next successful reconnect
    if (!staRetryStarted) {
      staRetryStarted = true;
...
void TaskSensor(void*){
  ...
  for(;;){
    ...
    // If else for auto re-detect if sensor is unplugged/plugged again
    // If hasX is true but i2cPresent fails => sensor lost
    // If hasX is false but i2cPresent ok => try to init again
    ...
    // Read previous fanOn state
    // Also read whether fan is manually disabled from web UI (if disabled, we keep fan off even if conditions say ON)
    ...
    // If dewA - dewB > DELTA_ON_HYST  => fan ON
    // If dewA - dewB <= DELTA_OFF_EDGE => fan OFF
...
void TaskDisplay(void*){
  for(;;){
    ...
    // Read shared state for display; if mutex not available, show defaults (mostly "--" and "OFF")
    if (g_stateMutex) {
      xSemaphoreTake(g_stateMutex, portMAX_DELAY);
      local = g_state;
      xSemaphoreGive(g_stateMutex);
...
void setup(){
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    apStarted = true;
    staRetryStarted = true;  // Start retry immediately in case WiFi appears later

    net_Mode = "AP";
    net_IP   = WiFi.softAPIP().toString();

    // Retry STA in background (in case WiFi appears later)
    WiFi.begin(WIFI_SSID, WIFI_PASS);  
  }
  ...
  // /startLogging with body: epoch=SECONDS in UNIX time (seconds since 1970-01-01 00:00:00 UTC)
  ...
  server.on("/startLogging", HTTP_POST, [](AsyncWebServerRequest* req){
    //if time is already valid (for example from NTP in STA mode),
    // do not overwrite it with browser time
    if (!timeValid()) {
      ...
      // Basic sanity check for epoch (must be > 2024-11-13)
      time_t epoch = (time_t) strtoll(req->getParam("epoch", true)->value().c_str(), nullptr, 10);
      if (epoch < 1700000000) {
      ...
      // this is only used as fallback when NTP has not already set valid time
  ...
  server.on("/data", HTTP_GET, [](AsyncWebServerRequest* req){
    ...
    j+="\"disabled\":";  j+= (local.fanDisabled ?"1":"0"); j+=",";
    j+="\"netMode\":\""; j+= net_Mode; j+="\",";
    j+="\"netIP\":\"";   j+= net_IP;   j+="\"";
    j+="}";
  ...
  server.on("/sd/days", HTTP_GET, [](AsyncWebServerRequest* req){
    ...
    // Iterate files in root, keep only directories (day folders)
  ...
  server.on("/sd/files", HTTP_GET, [](AsyncWebServerRequest* req){
    // Expect "day=DD-MM-YYYY" query parameter
    if (!req->hasParam("day")) {
    ...
    // Iterate files in the day folder, keep only files (no subdirs)
  ...
  server.on("/sd/raw", HTTP_GET, [](AsyncWebServerRequest* req){
    // Expect "day=DD-MM-YYYY" and "file=FILENAME" query parameters
  ...
  server.on("/sd/status", HTTP_GET, [](AsyncWebServerRequest* req){
    ...
    // Get purge info (next purge epoch, days left, whether to warn)
  ...
  // ================= RTOS init =================
  // Create mutex and queue before starting tasks
